...
describe("Unit Tests for IdentityRegistry", () => {
  ...
  describe("Initialization", () => {
    ...
    it("should not allow initialization after initialized", async () => {
      ...
      await expect(registry.initialize(hub.target)).to.be.revertedWithCustomError(
        registry,
        "InvalidInitialization",
      );
    });
  ...
  describe("Update functions", () => {
    ...
    it("should able to add dsc key commitment by owner", async () => {
      ...
      const event = receipt?.logs.find(
        (log) =>
          log.topics[0] === registry.interface.getEvent("DevDscKeyCommitmentRegistered").topicHash,
      );
      const eventArgs = event
    ...
    it("should able to update dsc key commitment by owner", async () => {
      ...
      const event = receipt?.logs.find(
        (log) =>
          log.topics[0] === registry.interface.getEvent("DevDscKeyCommitmentUpdated").topicHash,
      );
      const eventArgs = event
    ...
    it("should able to remove dsc key commitment by owner", async () => {
      ...
      const event = receipt?.logs.find(
        (log) =>
          log.topics[0] === registry.interface.getEvent("DevDscKeyCommitmentRemoved").topicHash,
      );
      const eventArgs = event
    ...
    it("able to change nullifier state by owner", async () => {
      ...
      const event = receipt?.logs.find(
        (log) =>
          log.topics[0] === registry.interface.getEvent("DevNullifierStateChanged").topicHash,
      );
      const eventArgs = event
    ...
    it("able to change dsc key commitment state by owner", async () => {
      ...
      const event = receipt?.logs.find(
        (log) =>
          log.topics[0] ===
          registry.interface.getEvent("DevDscKeyCommitmentStateChanged").topicHash,
      );
      const eventArgs = event
        ? registry.interface.decodeEventLog(

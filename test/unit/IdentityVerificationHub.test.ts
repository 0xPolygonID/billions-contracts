...
describe("Unit Tests for IdentityVerificationHub", () => {
    ...
    describe("Initialization", () => {
        it("should initialize hub with correct parameters", async () => {
            const {hub, registry, vcAndDisclose, signature, dsc} = deployedActors;
            ...
            const signatureId = SignatureVerifierId.signature_sha256_sha256_sha256_rsa_65537_4096;
            const dscId = DscVerifierId.dsc_sha256_rsa_65537_4096;
            expect(await hub.sigTypeToSignatureCircuitVerifiers(signatureId)).to.equal(signature.target);
            ...
            expect(hubInitializedEvent.args.signatureCircuitVerifierIds).to.deep.equal([signatureId]);
            expect(hubInitializedEvent.args.signatureCircuitVerifiers).to.deep.equal([signature.target]);
    ...
    describe("Update functions", () => {
        ...
        it("should update register circuit verifier", async () => {
            const { hub, user1 } = deployedActors;
            const verifierId = SignatureVerifierId.signature_sha256_sha256_sha256_rsa_65537_4096;
            const newVerifierAddress = await user1.getAddress();

            await expect(hub.updateSignatureCircuitVerifier(verifierId, newVerifierAddress))
                .to.emit(hub, "SignatureCircuitVerifierUpdated")
                .withArgs(verifierId, newVerifierAddress);

            expect(await hub.sigTypeToSignatureCircuitVerifiers(verifierId)).to.equal(newVerifierAddress);
        });
        ...
        it("should not update register circuit verifier if caller is not owner", async () => {
            ...
            await expect(hub.connect(user1).updateSignatureCircuitVerifier(verifierId, newVerifierAddress)).to.be.revertedWithCustomError(hub, "OwnableUnauthorizedAccount");
        });
        ...
        it("should not update register circuit verifier if caller is not proxy", async () => {
            ...
            await expect(hubImpl.updateSignatureCircuitVerifier(verifierId, newVerifierAddress)).to.be.revertedWithCustomError(hubImpl, "UUPSUnauthorizedCallContext");
        });
        ...
        it("should batch update register circuit verifiers", async () => {
            ...
            await expect(hub.batchUpdateSignatureCircuitVerifiers(verifierIds, newVerifierAddresses))
                .to.emit(hub, "SignatureCircuitVerifierUpdated")
            ...
            for (let i = 0; i < verifierIds.length; i++) {
                expect(await hub.sigTypeToSignatureCircuitVerifiers(verifierIds[i]))
        ...
        it("should not batch update register circuit verifiers if caller is not owner", async () => {
            ...
            await expect(hub.connect(user1).batchUpdateSignatureCircuitVerifiers(verifierIds, newVerifierAddresses)).to.be.revertedWithCustomError(hub, "OwnableUnauthorizedAccount");
        });
        ...
        it("should not batch update register circuit verifiers if caller is not proxy", async () => {
            ...
            await expect(hubImpl.batchUpdateSignatureCircuitVerifiers(verifierIds, newVerifierAddresses))
        ...
        it("should not batch update register verifiers if length is not the same", async () => {
            ...
            await expect(
                hub.batchUpdateSignatureCircuitVerifiers(verifierIds, newVerifierAddresses)
    ...
    describe("View functions", () => {
        ...
        it("should return correct signature circuit verifier address", async () => {
            const { hub, signature } = deployedActors;
            const verifierId = SignatureVerifierId.signature_sha256_sha256_sha256_rsa_65537_4096;
            expect(await hub.sigTypeToSignatureCircuitVerifiers(verifierId)).to.equal(signature.target);
        });

        it("should not return when view function is called by non-proxy", async () => {
            const { hubImpl } = deployedActors;
            await expect(hubImpl.sigTypeToSignatureCircuitVerifiers(1)).to.be.revertedWithCustomError(hubImpl, "UUPSUnauthorizedCallContext");
        });
    ...
    describe("Upgradeabilitiy", () => {
        it("should preserve state after upgrade", async () => {
            ...
            const signatureCircuitVerifierIdsBefore = await hub.sigTypeToSignatureCircuitVerifiers(
            ...
            expect(await hubV2.sigTypeToSignatureCircuitVerifiers(
                SignatureVerifierId.signature_sha256_sha256_sha256_rsa_65537_4096
            )).to.equal(signatureCircuitVerifierIdsBefore);
